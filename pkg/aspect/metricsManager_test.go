...
func TestMetricsManager_NewAspect(t *testing.T) {
	...
	if _, err := newMetricsManager().NewReportExecutor(conf, f, atest.NewEnv(t), df, nil); err != nil {
...
func TestMetricsManager_NewAspect_PropagatesError(t *testing.T) {
	...
	f, _ := FromBuilder(builder, config.MetricsKind)
	_, err := newMetricsManager().NewReportExecutor(conf, f, atest.NewEnv(t), df, nil)
	if err == nil {
