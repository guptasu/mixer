...
func (m *fakeCheckAspectMgr) NewCheckExecutor(
	cfg *cpb.Combined, _ aspect.CreateAspectFunc, env adapter.Env, _ descriptor.Finder, _ template.Repository) (aspect.CheckExecutor, error) {
	m.called++
	if m.ce == nil {
		return nil, errors.New("unable to create aspect")
	}
...
func (m *fakeReportAspectMgr) NewReportExecutor(
	cfg *cpb.Combined, _ aspect.CreateAspectFunc, env adapter.Env, _ descriptor.Finder, _ template.Repository) (aspect.ReportExecutor, error) {
	m.called++
	if m.re == nil {
		return nil, errors.New("unable to create aspect")
	}
...
func (m *fakeQuotaAspectMgr) NewQuotaExecutor(
	cfg *cpb.Combined, _ aspect.CreateAspectFunc, env adapter.Env, _ descriptor.Finder, _ template.Repository) (aspect.QuotaExecutor, error) {
	m.called++
	if m.qe == nil {
		return nil, errors.New("unable to create aspect")
	}
